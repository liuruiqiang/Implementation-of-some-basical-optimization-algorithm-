...
class optimizer(object):
    ...
    def f(self,x):
        ...
        '''
        x1 ,x2 = x[0],x[1]
        return x1**2 + 3*x2**2 - 4*x1 -2*x1*x2
    ...
    def unknown_conjugate_gradient(self,x0,epoches=1000,eps=1e-6,rph = 0.1,init_alpha = 1,t = 2):
        '''
        针对函数变量不定的情况给出的共轭梯度法优化过程
        ...
        while(i < epoches and delta >eps):
            ...
            belta = self.get_belta(grad,grad0)
            grad0 = grad.flatten()
            d = -grad0 + belta * d
            sym_fx,npfx = self.unknown_f(symx,x)
    ...
    def __call__(self,x0):
        if self.f_mode == 'f':
            final_x,epoches,records_f,records_g = self.conjugate_gradient(x0)
            return final_x,epoches,records_f,records_g
        else:
            final_x,epoches,records_f,records_g,records_x,record_normG = self.unknown_conjugate_gradient(x0)
            return final_x,epoches,records_f,records_g,records_x,record_normG



    def conjugate_gradient(self,x0,epoches=1000,eps=1e-3):
        fx0 = self.f(x0)
        grad0 = self.g(x0)
        lens = len(x0)
        d = -grad0.flatten()
