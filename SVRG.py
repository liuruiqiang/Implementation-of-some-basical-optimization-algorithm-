...
class softmax_regression():
    def __init__(self, x_train, x_test, y_train, y_test, lr=0.01, epoches=200, inner_epoches=50,batch_size=1):
        """
        ...
        self.inner_epoches = inner_epoches
    ...
    def train(self,x,y,upgrade_mode='a'):
        """
        ...
        X, Y = x, y
        step = 0
        #saved_grads用于保存所有样本的梯度值
        saved_grads = np.zeros((n, self.input_dim, self.output_dim))
        test_loss_list = []
        test_acc_list = []
        grad_list = []
        train_loss_list = []
        ...
        for i in range(n):
            ...
            grads = np.dot(np.expand_dims(x[i], axis=0).T, (pred_y - np.expand_dims(y[i], axis=0)))
            saved_grads[i] = grads
        while (step < self.epoches):
            ...
            #计算全体样本梯度的平均值
            R_grads = np.dot(X.T, (pred_y - Y)) / n
            inner_w = self.w
            inner_grads = np.zeros((self.inner_epoches,self.input_dim, self.output_dim))
            #内层循环
            for j in range(self.inner_epoches):
                i_j = np.random.randint(0, n)
                x, y = np.expand_dims(X[i_j], 0), np.expand_dims(Y[i_j], 0)
                innner_pred_y = np.dot(x, inner_w)
                inner_grad = np.dot(x.T,(innner_pred_y - y))
                inner_grads[j] = inner_grad
                pred_y = np.dot(x, self.w)
                grad = np.dot(x.T,(pred_y - y))
                g_j = inner_grad - (grad - R_grads)
                #更新内层循环的参数w
                inner_w = inner_w - self.lr * g_j
            if upgrade_mode == 'a':
                self.w = inner_w
            elif upgrade_mode == 'b':
                self.w = np.mean(inner_grads,axis=0)
            else:
                k = np.random.randint(0, self.inner_epoches)
                self.w = inner_grads[k]
...
if __name__ == "__main__":
    ...
    # model.train(x_train, y_train)
    # acc, loss = model.test(x_test, y_test)
    # model.plot_test_loss(model.test_loss_list)
    # model.plot_train_loss(model.train_loss_list)
    # model.plot_accuracy()
    # # model.plot_grad()
    # model.plot_test_errors(model.test_error_list)
