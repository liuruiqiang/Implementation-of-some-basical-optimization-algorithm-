...
class softmax_regression():
    ...
    def update(self,grads):
        '''
        :param grads:
        :return: Nadam对梯度改进后得到的参数修改项
        '''
        self.s = self.s * self.alpha + (1 - self.alpha) * grads
        self.r = self.r * self.beta + (1 - self.beta) * np.square(grads)
        self.alpha_i *= self.alpha
        self.beta_i *= self.beta
        lr = -self.lr * (1 - self.beta_i) ** 0.5 / (1 - self.alpha_i)
        return lr * (self.s * self.alpha + (1 - self.alpha) * grads) / np.sqrt(self.r)
    ...
    def train_on_batch(self,x,y):
        ...
        while(step < self.epoches):
            ...
            # print(grads,self.s,self.r,self.alpha_i,self.beta_i)
