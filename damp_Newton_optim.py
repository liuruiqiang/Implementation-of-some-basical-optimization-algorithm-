...
class optimizer(object):
    ...
    def unknown_damp_newton_opt(self,x0,epoches=1000,eps=1e-6):
        lens = len(x0)
        ...
        while(i < epoches and delta >eps):
            ...
            Gx_reverse = np.linalg.inv(Gx)
            dk = np.dot(-Gx_reverse,grad)
            alpha = self.unknownf_search_alpha(symx,x,dk,self.rph,self.init_alpha,self.t)
            # alpha = self.search_alpha(x,dk,self.rph,self.init_alpha,self.t)
            x = x + alpha * dk
            record_x[:,i] = x
            sym_fx,npfx = self.unknown_f(symx,x)
    ...
    def __call__(self,x0):
        if self.f_mode == 'f':
            ...
            final_x,epoches,records_f,records_g,records_x,records_normG = self.unknown_damp_newton_opt(x0)
